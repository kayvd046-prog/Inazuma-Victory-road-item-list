...
A searchable index of 2,048 items in *Inazuma Eleven: Victory Road* and where each one comes
...
- **Build a set** puts a character next to their gear: search one of 5,206 characters, switch
  ...
  base, what the gear adds, and the total, and it adds up what the four items cost in tokens.
  Each gear slot is a search list that shows the stats of every item, and the menu above it ranks
  all four lists by any stat you like. The character, the version, the level and the set live in
  the URL, so a build can be shared as a link.
...
The list started from a community document of March 2026. The 177 moves, equipment and tactics that
...
in the token prices of 1,245 items and the 74 moves whose power was changed by later patches. Kits
...
their level 50 and level 99 stats and the Hero and Fabled versions where the game has them. It is
generated by `tools/build-characters.py` from the datamined dump. It is fetched only when the builder is opened, so it costs nothing on a
