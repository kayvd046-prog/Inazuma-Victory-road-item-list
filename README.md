...
- **Build a set** puts a character next to their gear: search one of 5,206 characters, switch
  ...
  base, what the gear adds, and the total. It also lists the moves that character learns and at
  which level — each one a link into the list — and adds up what the four items cost in tokens.
...
their level 50 and level 99 stats, the Hero and Fabled versions where the game has them, and the
moves each one learns with the level attached. It is generated by `tools/build-characters.py` from
the datamined dump. It is fetched only when the builder is opened, so it costs nothing on a
